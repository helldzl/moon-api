 ...
 */
@Service
public class QuestionsServiceImpl extends AbstractBaseService<Questions, QuestionsDao> implements QuestionsService {
    @Autowired
    ...
    public Page<Questions> findAlll(Long quizId,int page,String sessionCode,List<Field> fields){
        ...
        if(pages.hasContent()) {
            ...
            Long[] questionIds = questions.stream().map(Questions::getId).collect(Collectors.toList()).toArray(new Long[questions.size()]);
            List<Options> options = Services.findAll(Options.class, 
                    Restrictions.in(Options.QUESTION_ID, questionIds),
                    fields);
            Map<Long,List<Options>> map = options.stream().collect(Collectors.groupingBy(Options::getQuestionId));
            questions.stream().forEach(q -> q.setOptions(map.get(q.getId())));
        }
