...
@Api(description = "前台接口-中奖晒单-shares相关API", basePath = "/shares")
@RestController
@RequestMapping("/shares")
public class SharesController extends RestfulController<Shares> {

    @ApiOperation(value = "根据id获取指定晒单内容")
    @ApiImplicitParams({
    ...
    @ApiOperation(value = "分页获取晒单记录", notes = "添加筛选条件和排序字段 filter[enabled]=1&filter[prizeId]=1&filter[userId]")
    @ApiImplicitParams({
    ...
    @Override
    public ResponseEntity<Response> doPost(Data<Shares> data) {
        if(Services.findOne(Prizes.class,data.getData().getPrizeId()) == null) {
