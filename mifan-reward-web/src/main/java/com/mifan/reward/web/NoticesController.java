...
@Api(description = "前台接口-公告信息-notices相关API", basePath = "/notices")
@RestController
@RequestMapping("/notices")
public class NoticesController extends RestfulController<Notices> {

    @ApiOperation(value = "根据id获取指定公告内容")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "path", name = "id", dataType = "long", value = "公告id")
    })
    @Override
    ...
    @ApiOperation(value = "分页获取系统公告", notes = "1.添加筛选条件和排序字段 filter[enabled]=1&filter[state]>1&sort=display_order 2.state(1 默认 2 首页显示 3 首页不显示)")
    @ApiImplicitParams({
